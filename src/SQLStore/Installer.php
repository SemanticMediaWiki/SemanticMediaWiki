...
class Installer implements MessageReporter {
	...
	public function install( $verbose = true ) {
		...
		$messageReporter->reportMessage( "\nSetting up database tables for SMW ...\n\n" );
	...
	public function uninstall( $verbose = true ) {
		...
		$messageReporter->reportMessage( "\nSelected storage engine: \"SMWSQLStore3\" (or an extension thereof)\n" );
		$messageReporter->reportMessage( "\nChecking database tables generated by SMW ...\n" );
		...
		foreach ( $this->tableSchemaManager->getOrphanedTables() as $table ) {
			$this->tableBuilder->drop( $table );
		}
		...
		$messageReporter->reportMessage( "\nAll Semantic MediaWiki tables (incl. auxiliary tables\n" );
		$messageReporter->reportMessage( "and corresponding data) have been removed successfully.\n" );
