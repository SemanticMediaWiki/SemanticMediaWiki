...
class Search extends SearchEngine {
	...
	public function getFallbackSearchEngine() {
		if ( $this->fallbackSearch === null ) {
			...
			$this->assertIsSearchEngineClassName( $type );
	 ...
	 * @return string
	 */
	private function getDefaultSearchEngineTypeForDB( DatabaseBase $db ) {
	 ...
	 */
	private function assertIsSearchEngineClassName( $className ) {
		if ( !class_exists( $className ) ) {
			throw new RuntimeException( "$className does not exist." );
		}

		if ( !is_subclass_of( $className, 'SearchEngine' ) || $className === 'SMWSearch' ) {
			throw new RuntimeException( "$className is not a valid fallback search engine type." );
		}
	}
