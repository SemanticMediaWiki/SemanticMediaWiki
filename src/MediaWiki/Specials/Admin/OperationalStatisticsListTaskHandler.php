...
class OperationalStatisticsListTaskHandler extends TaskHandler {
		...
		return Html::rawElement(
			...
			$this->getMessageAsString(
				[
					...
					$this->outputFormatter->getSpecialPageLinkWith( $this->getMessageAsString( 'smw-admin-supplementary-operational-statistics-title' ), [ 'action' => 'stats' ] )
				]
			)
		);
	}
	...
	private function outputSemanticStatistics() {
		...
		$this->outputFormatter->addHTML(
			Html::rawElement( 'p', [], $this->getMessageAsString( [ 'smw-admin-operational-statistics' ], Message::PARSE ) )
		);

		$this->outputFormatter->addHTML(
			Html::element( 'h2', [], $this->getMessageAsString( 'semanticstatistics' ) )
		);
	...
	private function outputJobStatistics() {
		...
		$this->outputFormatter->addHTML(
			Html::element( 'h2', [], $this->getMessageAsString( 'smw-admin-statistics-job-title' ) )
		);

		$this->outputFormatter->addHTML(
			Html::rawElement( 'p', [], $this->getMessageAsString( 'smw-admin-statistics-job-docu', Message::PARSE ) )
		);
		...
		$this->outputFormatter->addHTML(
			Html::rawElement(
				...
				Html::element( 'div', [ 'class' => 'smw-admin-statistics-job-content' ], $this->getMessageAsString( 'smw-data-lookup' ) )
			)
		);
	}
	...
	private function outputQueryCacheStatistics() {
		...
		$this->outputFormatter->addHTML(
			Html::element( 'h2', [], $this->getMessageAsString( 'smw-admin-statistics-querycache-title' ) )
		);
		...
		if ( !$cachedQueryResultPrefetcher->isEnabled() ) {
			return $this->outputFormatter->addHTML(
				Html::rawElement( 'p', [], $this->getMessageAsString( [ 'smw-admin-statistics-querycache-disabled' ], Message::PARSE ) )
			);
		}

		$this->outputFormatter->addHTML(
			Html::rawElement( 'p', [], $this->getMessageAsString( [ 'smw-admin-statistics-querycache-explain' ], Message::PARSE ) )
		);
