...
A detailed list of settings and their explanations is available in [`docs/config.md`](../../../docs/config.md) (the per-setting reference) and `extension.json`'s `config` block (the authoritative defaults). Please make sure that after changing any setting, `php rebuildElasticIndex.php --update-settings` is executed.
