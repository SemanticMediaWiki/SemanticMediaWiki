...
/**
 * Pure content generator for the seedDemoData maintenance script. Holds the
 * static breed/topic/category data and renders it to wikitext. Has no
 * MediaWiki dependencies, so it can be unit tested in isolation.
 *
 * @license GPL-2.0-or-later
 * @since 7.0.0
 */
final class DemoContentBuilder {

	private const BREED_GROUPS = [
		'Herding', 'Sporting', 'Hound', 'Terrier', 'Toy', 'Working', 'Non-Sporting',
	];

	private ?array $breeds = null;
	private ?array $topicPages = null;
	 ...
	 */
	public function getBreedData(): array {
		$this->breeds ??= $this->buildBreedData();
		return $this->breeds;
	}
	 ...
	 */
	public function getTopicPages(): array {
		$this->topicPages ??= $this->buildTopicPages();
	...
	public function getCategoryData(): array {
		$breeds = $this->getBreedData();
		$categories = [];

		// Breed group categories
		foreach ( self::BREED_GROUPS as $group ) {
			$breedsInGroup = array_filter( $breeds, static fn ( $b ) => $b['group'] === $group );
			$breedList = implode( "\n", array_map( static fn ( $b ) => "* [[{$b['name']}]]", $breedsInGroup ) );
			$desc = $this->renderCategoryDescription( $group );
		...
		$origins = array_unique( array_column( $breeds, 'origin' ) );
	 ...
	 * @since 7.0.0
	 */
	public function renderBreedTemplate(): string {
		return '<noinclude>
...
|}</includeonly>';
	 ...
	 * @since 7.0.0
	 */
	public function renderCategoryDescription( string $group ): string {
		$descriptions = [
...
[[Category:Seed data]]
	 ...
	 * @return array<int, array<string, mixed>>
	 */
	private function buildBreedData(): array {
		return [
			// Herding
	 ...
	 * @return array<int, array{title: string, content: string}>
	 */
	private function buildTopicPages(): array {
		return [
			[
				'title' => 'Dog Training',
