...
//// Methods for adding data to the object
	...
	static function addAttribute($attribute, $value) {
		...
		switch ($special) {
			case false: // normal attribute
				$result = SMWDataValue::newAttributeValue($attribute,SMWFactbox::$skin,$value);
				if ($smwgStoreActive) {
			...
			default: // generic special attribute
				if ( $special === SMW_SP_SERVICE_LINK ) { // do some custom formatting in this case
					...
					$result = SMWDataValue::newSpecialValue($special,SMWFactbox::$skin,$v);
					...
					$result = SMWDataValue::newSpecialValue($special,SMWFactbox::$skin,$value);
				}
				if ($smwgStoreActive) {
	...
	static private function addImportedDefinition($value) {
		...
		if ( count($msglines) < 2 ) { //error: no elements for this namespace
			$datavalue = SMWDataValue::newTypedValue(new SMWErrorTypeHandler(wfMsgForContent('smw_unknown_importns',$onto_ns)),SMWFactbox::$skin,$value);
		...
		if (NULL != $error) {
			$datavalue = SMWDataValue::newTypedValue(new SMWErrorTypeHandler($error),SMWFactbox::$skin,$value);
			if (!$smwgStoreActive) {
		...
		if (!$smwgStoreActive) {
			$datavalue = SMWDataValue::newTypedValue($sth,SMWFactbox::$skin,$onto_uri);
			SMWFactbox::$semdata->addSpecialValue(SMW_SP_EXT_BASEURI,$datavalue);
			$datavalue = SMWDataValue::newTypedValue($sth,SMWFactbox::$skin,$onto_ns);
			SMWFactbox::$semdata->addSpecialValue(SMW_SP_EXT_NSID,$datavalue);
			$datavalue = SMWDataValue::newTypedValue($sth,SMWFactbox::$skin,$onto_section);
		...
		$datavalue = SMWDataValue::newTypedValue($sth,SMWFactbox::$skin,"[$onto_uri$onto_section $value]");
