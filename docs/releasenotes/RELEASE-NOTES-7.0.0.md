...
* Removed the `setFingerprint()` and `getFingerprint()` methods plus the static fingerprint-based queue-dedup branch from `SMW\MediaWiki\Deferred\CallableUpdate` (also inherited by `TransactionalCallableUpdate`, which has no override). The dedup was a 2016 speculative perf optimisation: it skipped queueing a second `pushUpdate()` call carrying the same fingerprint. Every production caller (`DataUpdater`, `Query\Cache\ResultCache`, `SQLStore\QueryDependency\QueryDependencyLinksStore`, `Utils\Stats`, `MediaWiki\PageUpdater`) schedules an idempotent callback, so re-running a duplicate update only wastes work; it does not produce incorrect data. `QueryDependencyLinksStore` retains its existing `isRegistered( $sid, $subject )` in-memory dedup, which was always the primary gate. `SMW\MediaWiki\PageUpdater::setFingerprint()` is also removed: it had no production callers and silently collided every production `PageUpdater` instance on `md5('')`. The static `CallableUpdate::$queueList` had no clear-between-tests path and leaked on a throwing callback, so it was a latent test-isolation hazard. As a side effect, the unused private `$fingerprint` property on `SMW\Utils\Stats` is also gone; callers that need their own dedup should track it themselves.
