...
* The common `#ask` presentation formats (`table`, `broadtable`, `list`, `ul`, `ol`, `plainlist`, `template`, `embedded`, `category`) no longer fragment the parser cache by `userlang` for successful results. The only interface-language-dependent part of such a result, the "further results" link label (and, for `category`, the column-continuation abbreviation), is now emitted as a language-neutral placeholder during the parse and resolved per request after the parser cache, so one cached entry serves every interface language. Queries that produce errors still vary by `userlang`, since error messages are localised. On multilingual wikis this substantially improves the parser cache hit rate for query-driven pages.
