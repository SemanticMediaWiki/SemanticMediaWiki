...
* Fixed `smw_query_links` losing printout-derived dependencies when a page embedded two or more `#ask` queries that shared a query-id hash (same conditions, different printouts). `Query::getHash()` deliberately excludes printouts, so the affected queries resolved to the same `$sid` and the per-callback `DELETE`-then-`INSERT` flush had each callback overwrite the previous one. `QueryDependencyLinksStore` now buffers per-callback work and coalesces the flush into a single end-of-request `DependencyLinksTableUpdater::doUpdate()`, so all affected queries' printout properties survive in `smw_query_links`. Changes to those properties once again invalidate the embedding page's cached query results.
