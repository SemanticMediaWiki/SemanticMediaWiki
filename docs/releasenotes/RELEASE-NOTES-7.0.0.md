...
* **`loadDefaultConfigFrom()` removed.** The method on the return value of `enableSemantics()` has been removed and will fatal if called. Replace with a direct `require`:
  ...
  **Correction:** this `require` does not apply the profile, and fails with an error for `media.php`. Since 7.3.0, list the profile in `$smwgConfigProfiles` instead ([#7106](https://github.com/SemanticMediaWiki/SemanticMediaWiki/issues/7106)).
