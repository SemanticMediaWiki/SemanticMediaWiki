...
* `rebuildData.php` accepts a new `--use-job` option that queues its update jobs instead of running them inline, so a full rebuild can be processed in parallel with `runJobs.php --type smw.update --procs N`. The per-entity parse cost is unchanged, so set the worker count to what your database can absorb.
