...
* #1106 Added `--skip-properties` flag to `rebuildData.php` / remove marked for deletion first
...
* #1171 Extended `format=debug` to output the query EXPLAIN report that is generated by the SQL engine (those which support this feature) to allow for a better comprehension about the overall `#ask` execution
