...
function doSpecialFocus($query = '') {
	SMW_SpecialFocus::execute($query);
}

SpecialPage::addPage( new SpecialPage('Focus','',true,'doSpecialFocus',false) );



class SMW_SpecialFocus	 {
	...
	static function execute($query = '') {
		...
		$html = '';
		$spectitle = Title::makeTitle( NS_SPECIAL, 'Focus' );

		// display query form
		$html .= '<form name="smwfocus" action="' . $spectitle->escapeLocalURL() . '" method="get">' . "\n";
		...
		if ((NULL == $article) || ('' == $articletext)) { // empty, no article name given
			...
			if ($offset > 0)
				$navigation = '<a href="' . htmlspecialchars($skin->makeSpecialUrl('Focus','offset=' . max(0,$offset-$limit) . '&article=' . urlencode($articletext) )) . '&mode=in">' . wfMsg('smw_result_prev') . '</a>';
			else
			...
			if (count($inrel)==($limit+1))
				$navigation .= ' <a href="' . htmlspecialchars($skin->makeSpecialUrl('Focus', 'offset=' . ($offset+$limit) . '&article=' . urlencode($articletext) ))  . '&mode=in">' . wfMsg('smw_result_next') . '</a>';
			else
				...
				foreach ($inrel as $result) {
					...
					foreach ($subjects as $subject) {
						...
						if (($innercount < $innerlimit) || !$more) {
							$subjectlink = SMWInfolink::newFocusLink('+',$subject->getFullText());
			...
			if ((count($outrel) == 0) && (count($atts) == 0)) {
				...
				foreach ($outrel as $result) {
					...
					foreach ($objects as $object) {
						$count += 1;
						$searchlink = SMWInfolink::newFocusLink('+',$object->getFullText());
