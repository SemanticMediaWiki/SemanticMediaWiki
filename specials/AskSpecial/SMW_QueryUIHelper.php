	...
	protected function enableJQuery() {
		global $wgOut, $smwgJQueryIncluded;
			if ( !$smwgJQueryIncluded ) {
				$real_function = array( 'OutputPage', 'includeJQuery' );
...
EOT;
	...
	protected function getUrlTail() {
		$url_tail = '&q=' . urlencode( $this->uiCore->getQuerystring() );
		...
		$url_tail .= '&p=' . urlencode( SMWInfolink::encodeParameters( $tmp_parray ) );

		$print_out_string = '';
		foreach ( $this->uiCore->getPrintOuts() as $printout ) {
			$print_out_string .= $printout->getSerialisation() . "\n";
		}

		if ( $print_out_string != '' ) {
			$url_tail .= '&po=' . urlencode( $print_out_string );
		}
		if ( array_key_exists( 'sort', $params ) ) {
			$url_tail .= '&sort=' . $params['sort'];
		}
		if ( array_key_exists( 'order', $params ) ) {
			$url_tail .= '&order=' . $params['order'];
		}
	...
	protected function showFormatOptions( $format, array $paramValues, array $ignoredAttribs = array() ) {
		...
		foreach ( $params as $param ) {
			...
			$current_value = array_key_exists( $param->getName(), $paramValues ) ? $paramValues[$param->getName()] : false;
			...
			$options_html[] =
				Html::rawElement(
					...
					$this->showFormatOption( $param, $current_value ) .
					'<br />' .
					Html::element( 'em', array(), $param->getDescription() )
				);
		}

		for ( $i = 0, $n = count( $options_html ); $i < $n; $i++ ) {
			if ( $i % 3 == 2 || $i == $n - 1 ) {
				$options_html[$i] .= "<div style=\"clear: both\";></div>\n";
			}
		}
		...
		$flip_style = true;
		$flip_count = 0;
		while ( $option = array_shift( $options_html ) ) {
			$row_html .= $option;
			$i++;

			$result_html .= Html::rawElement(
				'div',
				array(
					'style' => 'background: ' . ( $flip_style ? 'white' : '#dddddd' ) . ';'
				),
				$row_html
			);
...
END;
	...
	public function getPOStrings() {
		$string = '';
		$print_outs = $this->uiCore->getPrintOuts();
		if ( !empty( $print_outs ) ) {
			foreach ( $print_outs as $value ) {
...
class SMWQueryUIHelper {
	 ...
	 */
	public function setPrintOuts( array $print_outs = array(), $enableValidation = false ) {
		/*
	...
	public function extractParameters( $p ) {
		if ( $this->context == self::SPECIAL_PAGE ) {
			...
			$raw_params = array_merge( $this->parameters, array( $this->queryString ), $this->printOutStrings );
		} else {// context is WIKI_LINK
			$raw_params = SMWInfolink::decodeParameters( $p, true );
			// calling setParams to fill in missing parameters
			$this->setParams( $raw_params );
			$raw_params = $this->parameters;
		}

		SMWQueryProcessor::processFunctionParams( $raw_params, $this->queryString, $this->parameters, $this->printOuts );
	}
	...
	public function execute() {
		...
		if ( $this->parameters['format'] == 'rss' ) {
			$desc_key = 'rssdescription';
			$titlekey = 'rsstitle';
		} elseif ( $this->parameters['format'] == 'icalendar' ) {
			$desc_key = 'icalendardescription';
			$titlekey = 'icalendartitle';
		} else {
			$desc_key = false;
		}

		if ( $desc_key && ( $query->getDescription() instanceof SMWConceptDescription ) &&
			 ( !isset( $this->parameters[$desc_key] ) || !isset( $this->parameters[$titlekey] ) ) ) {
			...
			if ( !isset( $this->parameters[$desc_key] ) ) {
				...
				if ( $dv instanceof SMWConceptValue ) {
					$this->parameters[$desc_key] = $dv->getDocu();
				}
			}
		}
