...
class SMWQueryCreatorPage extends SpecialPage {
	...
	/**
	 * Extracts Query and option parameters, especially q (the query), 
	 * offset and limit (number of results per-page), p (the result format and options), po (printouts) and 
	 * eq(show/hide the query editor).
	 * 
	...
	protected function extractQueryParameters( $p ) {
		...
		if ( $paramstring != '' ) { // parameters from HTML inputs
		...
		*/
		...
		$this->m_editquery = ( $wgRequest->getVal( 'eq' ) == 'no' ) || ( $this->m_querystring == '' );
	}
...
END;
	 ...
	 * @return string
	 */
	protected function getInputForm( $urltail ) {
		...
		if ( $this->m_editquery ) {
			...
			$result .= '<p><textarea name="q" rows="6">' . htmlspecialchars( $this->m_querystring ) . '</textarea></p>';
